    ...
    <div class="container mt-5">
            ...
            <div class="col-md-12">
                <div class="card-header mt-2">
                        <div style="float:right;padding-right:2rem;padding-top:.8rem;font-size:1.4rem;">
                                <a href=" {{ url('/admin/order/invoice/'.$orders->id) }} " class="btn btn-info">View Invoice</a>
                                <a href="{{ url('/admin/order/invoice/pdf/'.$orders->id) }}" target="_blank" class="btn btn-warning">Download Invoice</a>
                                <a href=" {{ url('/admin/order/all') }} " class="btn btn-info">Back</a>
                            </div>
                    <h1 >Order Details</h1>
                ...
                <div class="row" >
                    <div class="col-md-6 mt-2 my-5">
                        ...
                        <div class="order_details ">
                            ...
                            <span class="fs-7"> Payment Mode: <span class="badge rounded-pill bg-success"> @if ($orders->payment_method_cash)
                                Cash On Delivery
                            @else
                                Bkash Payment
                            @endif
                    ...
                    <div class="col-md-6 mt-2 my-5">
                        ...
                        <div class="order_details ">
                            <span class="fs-7"> Customer Name: {{ $orders->customer_name }}</span><br>
                            <span class="fs-7">Phone: {{ $orders->customer_phone }} </span> <br>
                        ...
                        @foreach ( (App\Models\Porder::where('time',$orders->time)->get()) as $item)
                            <tr class="mt-2">
                                <td class="shoping__cart__item my-2">
                                    <img class="rounded" src=" {{asset($item->product->product_img)}}" style="width:50px;height:50px" alt="" >
                                </td>
        ...
        <div class="row">
            @if(session('order_update')) 
                <div class="alert alert-success alert-dismissible fade show" role="alert">
                    <strong> {{ session('order_update') }} </strong>
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
            @endif
            <div class="fs-3">
            ...
            <div class="col-md-6 mt-3">
                ...
                <form action=" {{ url('admin/order/order_status/update/'.$orders->id) }} " method="POST">
                    @csrf
            ...
            <div class="col-md-6 mt-3 rounded">
                <div class="mt-5 order_status_msg fs-4 ps-2" >
                    Current Payment Method: <span class="badge rounded-pill bg-danger"> @if ($orders->payment_method_cash)
                        Cash On Delivery
                    @else
                        Bkash Payment
                    @endif </span>
